...
def draw_cdf(ds_sort,dataset_name):
    last, index = min(ds_sort), 0
    x = []
    y = []
    while index < len(ds_sort):
        x.append(ds_sort[index])
        y.append(index / len(ds_sort))
        if index < len(ds_sort) - 1:  # 确保不会越界
            last = ds_sort[index + 1]
        index += 1

    plt.figure(figsize=(8, 6))
    plt.scatter(x, y, color='purple', s=3, alpha=0.5)  # 使用scatter绘制散点图，s控制点的大小
    plt.xlabel(f'{dataset_name} Geolocation Error(km)')
    plt.ylabel('Cumulative Probability')
    plt.grid(True)
    plt.title(f'CDF of Geolocation Errors for {dataset_name}')
    plt.show()
